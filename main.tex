...
Our method of proof is simple: We first argue that there exists a
formula of the form
...
\end{align}
as in \autoref{theorem:main}.  Then, we determine the five
coefficients $a_{\bullet}$ by exhibiting particular good families for
which we {\sl a priori} know all $v_{i}$ and $\deg \mu$.  In this way,
we obtain a system of linear equations in five unknowns, which turns
out to have a unique solution.  In fact, we exhibit nine(?)  such
families/equations, leaving no doubt in the correctness of the
formula.
...
finite automorphism group.  The key technical point is that this
construction {\sl extends over the boundary of $\overline{\M}_{0,7}$,
  yielding a good family of cubic surfaces.}
...
result we will need in this section is:
...
$\overline{\sigma}_{0}, \dots, \overline{\sigma}_{\infty}$.
Importantly, $\sigma_{\infty}$ never intersects any of the other six
sections.  Also, observe that
