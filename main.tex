...
\subsection{Counting cubic surfaces}
The beautiful enumerative geometry of cubic surfaces has captured the
imagination of algebraic geometers for centuries. Yet, it is easy
to ask natural enumerative questions about cubic surfaces whose
solutions elude us.

One family of such questions concerns counting the moduli of cubic
surfaces in families. It is well-known that the isomorphism classes of
cubic surfaces form a 4-dimensional moduli space. Therefore, in any
4-dimensional family of cubic surfaces, a general isomorphism class is
expected to appear finitely many times. How do we determine this
number? For example, how many times does a general cubic surface
appear, up to isomorphism, as
\begin{enumerate}
\item a member of a 4-dimensional general linear system of cubic surfaces in $\mathbb
  P^3$?
...
To state our theorem, denote by $\mathcal M$ the coarse moduli space
of $\PGL(4)$-semistable cubic surfaces in $\mathbb P^3$.
By a family of cubic surfaces, we mean a flat, proper morphism whose
geometric fibers are isomorphic to cubic surfaces in $\mathbb P^3$.
\begin{theorem}\label{theorem:main}
  Let $\pi \colon X \to B$ be family of cubic surfaces.  Let $\V$ be
  the rank 4 bundle $\V = \pi_* \left(\omega_\pi^{-1}\right)$, and let
  \(v_i = c_i\left(\V\right)\) be its Chern classes.  Assume that
  every fiber of $\pi$ is good and a general fiber is GIT semi-stable.
  Let $\mu \colon B \dashrightarrow \mathcal M$ be the rational map
  ...
  \end{align}
\end{theorem}
\autoref{def:goodfamily} gives the precise meaning of ``good''. All
semi-stable surfaces and all surfaces (possibly singular) with a
finite automorphism group are good.

\subsection{Equivariant orbit classes}
