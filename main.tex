...
Then $\mathcal V = \chi(a) \oplus \chi(b) \oplus \chi(0) \oplus \chi(c)$.
The element $x_0x_1x_3 - x_2^3$ is a global section of $\sym^3 \mathcal V$.
In other words, $\mathcal X$ is a section of $\mathcal O_{\P \mathcal V}(3)$ and hence $M$ is trivial.
We get
\[ v_1 = 0, \quad v_2 = (ab+bc+ca)q^2, \quad v_3 = abc, \quad v_4 = 0,\]
and hence
...
{\color{red} Regarding checking the revised conjectural class in Anand's e-mail, I am having a more basic issue: I'm not getting integer coefficients when naively translating from $P$ to $Q$. The class $Q$ in \autoref{lem:Q_Zclass} (corresponding to the constant term of the equivariant Chow class of an orbit) is related to the class $P$ in \eqref{eq:P} by some linear transformation (given roughly by sustituting $t_i=t_i\pm \frac{t_0+\cdots+t_3}{3}$), and it's not clear to me that being divisible is preserved under this linear map. However, when naively trying to compute $Q$ from $P$, I didn't even get integral $P$... Roughly, I do something like

\begin{verbatim}
    SymmetricReduction[(-2840 v1^4 + 12440 (v1^2) v2 - 26800 (v1) (v3) + 
     57720 v4) /. {v1 -> t0 + t1 + t2 + t3, 
    v2 -> t0*t1 + t0*t2 + t0*t3 + t1*t2 + t1*t3 + t2*t3, 
    v3 -> t0*t1*t2 + t0*t1*t3 + t0*t2*t3 + t1*t2*t3, 
    v4 -> t0*t1*t2*t3} /. {t0 -> t0 + (t0 + t1 + t2 + t3)/3, 
   t1 -> t1 + (t0 + t1 + t2 + t3)/3, t2 -> t2 + (t0 + t1 + t2 + t3)/3,
    t3 -> t3 + (t0 + t1 + t2 + t3)/3}, {t0, t1, t2, t3}, {c1, c2, c3, 
  c4}]
\end{verbatim}
where I tried both possible signs. Not sure what I'm doing wrong yet.
}
