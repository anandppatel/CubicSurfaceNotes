...
Let $H = \Hilb^5(C) \cong \P^5$ and let $B_1 \cong \P^4 \subset H \cong \P^5$ be a general hyperplane.
Let $Z \subset B_1 \times C$ be the restriction of the universal closed subscheme.
Set 
\[Z' = Z \sqcup B_1 \times \{z_0\} \subset B_1 \times \P^2.\]
Our family
\[\pi \colon X \to B_1\]
is the family of cubic surfaces associated to $Z' \subset B_1 \times \P^2$ (see \autoref{def:cubicZ}).
It is easy to see that the family is good.
\begin{lemma}\label{lem:good1}
  The family $\pi \colon X \to B_1$ is good.
\end{lemma}
\begin{proof}
  We show that for every $b \in B_1$, the group $\Aut(X_b)$ is finite (see \autoref{ex:finaut}).
  ...
  Since $B_1 \subset \Hilb^5C$ is general, we may assume that it does not contain the finitely many length 5 schemes supported on $\Polar_C(z_0) \cap C$.
...
\begin{lemma}\label{lem:v1}
  Let $\mathcal V = \pi_* \left( \omega_{X/B_1}^{-1} \right)$.
  Then we have
  \[
...
where $\pi \colon B_1 \times \P^2 \to B_1$ is the first projection.
It contains the constant rank 2 bundle
\begin{equation}\label{eqn:v11}
  \pi_*\left( \mathcal I_{B_1 \times C} \otimes \mathcal I_{B_1 \times z_0} \otimes \omega_\pi^{-1} \right) = H^0\left(\mathcal I_{C} \otimes \mathcal I_{z_0} \otimes \omega_{\P^2}^{-1}\right) \otimes \O_{B_1},
\end{equation}
and the quotient is the rank 2 bundle $\pi_*\left( I_{Z/B_1 \times C} \otimes \omega_\pi^{-1} \right)$.
To identify the quotient, note that $Z \subset B_1 \times C$ is a divisor of class $(1,5)$ and $\omega^{-1}_{\pi}|_{B_1\times C}$ is isomophic to $\O(0,6)$.
Hence, $I_{Z/B_1 \times C} \otimes \omega_\pi^{-1}$ is isomorphic to $\O(-1,1)$, and therefore
\begin{equation}\label{eqn:v12}
  \begin{split}
    \pi_*\left( I_{Z/B_1 \times C} \otimes \omega_\pi^{-1} \right) \cong \O_{B_1}(-1)^{2}.
  \end{split}
\end{equation}
By combining \eqref{eqn:v11} and \eqref{eqn:v12}, we see that $\mathcal V$ is an extension
\begin{equation}\label{eqn:V1}
  0 \to \O_{B_1}^2 \to \mathcal V \to \O_{B_1}(-1)^2 \to 0.
\end{equation}
The Chern class calculation is not straightforward, using the Whitney sum formula.
\end{proof}

Let us find the degree of the map to moduli $B_1 \dashrightarrow \M$.
\begin{lemma}\label{lem:deg1}
  The degree of $B_1 \dashrightarrow \M$ is $4320$.
\end{lemma}
\begin{proof}
  ...
  The map $B_1 \dashrightarrow \M$ lifts to
  \begin{equation}\label{eqn:b1lift}
    B_1 \to \M^\dagger / S_5,
  \end{equation}
  where $b \in B_1$ is mapped to $(z_0, z_1 + \dots + z_5)|_b$.
  ...
  We must find out how many points of $B_1$ are equivalent to $z_1' + \dots + z_5'$ up to an automorphism of $\P^2$ preserving $(C, z_0)$.
  ...
  Then we have
   ...
   Hence, a general hyperplane $B_1$ intersects the orbit in 5 points.
   Accounting for the additional $\Z/2\Z$, we conclude that there are precisely 10 points of $B_1$ equivalent to $z_1' + \dots + z_5'$ up to an automorphism of $\P^2$ preserving $(C, z_0)$.
   Thus, the degree of the map in \eqref{eqn:b1lift} is 10.

   Since the degree of $\M^\dagger \to \M$ is $51840$, the degree of $B_1 \to \M$ is
...
We now make the construction more precise, and show that it can be extended to $\overline {\M}_{0,7}$.
...
Let $\mathcal L$ be the line bundle $\sigma_\infty^*\left(\omega_\phi\right)$ on $\overline \M_{0,7}$ defined by
...
Set $\overline \sigma_i = \sigma_i \circ \gamma$; these $\overline \sigma_i$ are sections of $\overline \phi \colon \overline C \to \overline \M_{0,7}$.
...
\[
\iota \colon \overline C \to \mathcal P := \P \Sym^2 \mathcal A^\vee.
\]
...
Let $\tau \colon \overline \M_{0,7} \to \mathcal P$ be the section obtained by taking fiberwise the pole with respect to $\overline {\mathcal C}$ of the line joining the points $\overline \sigma_0$ and $\overline \sigma_\infty$.
The direct sum decomposition $\mathcal A^\vee = \O \oplus \mathcal L$ gives the decomposition
\[ \Sym^2 \mathcal A^\vee = \O \oplus \mathcal L \oplus \mathcal L^2.\]
...
Using the quadratic form on $\Sym^2 A$ given by the Veronese conic, it is easy to check that their polar $\tau$ is given by the third projection $\Sym^2 A \to \mathcal L^{-1}$.

Since $\overline \sigma_0$ and $\overline \sigma_\infty$ are distinct in every fiber, the three points $\tau$, $\overline \sigma_0$, and $\overline \sigma_\infty$ are non-collinear in every fiber.
