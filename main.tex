...
\begin{proof}
  (1): The proposition follows after understanding the structure of
  the blowup
  $\mathcal{Y} = \Bl_{F} \left( \td{S^{[2]}} \times S \right)$. In
  order to do this, we must understand the normal bundle
  $N_{F/\left( \td{S^{[2]}} \times S \right)}$.  It suffices to focus
  on one component $F_{i} = E_{i} \times L_{i}$.  Clearly,
  $N_{F_{i}/\left( \td{S^{[2]}} \times S \right)} = \td{pi}_{1}^{*}
