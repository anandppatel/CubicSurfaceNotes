...
\label{sec:intro}
The primary goal of this paper is to determine an enumerative formula
which counts the number of times a general cubic surface occurs in
suitably general $4$-parameter families $ \pi: X \to B$ of cubic
surfaces.  We work over an algebraically closed, characteristic zero
ground field $k$.  Applying the formula to a few particular families
yields:
\begin{enumerate}
...
In simple terms, our method of proof is to first argue that there
...
we obtain a system of linear equations in five unknowns, and it turns
out to give a unique solution. We will actually exhibit nine(?)  such
families/equations, providing even further confidence in the
...
If $E$ is an $n+1$ dimensional vector space and if
...
enumerative problem and completely solved it when $n=1, 2$ and $d$ is
arbitrary.
...
This change in perspective opens the avenue of attack in this paper.
As will be seen in \autoref{section:???} we can exploit certain {\sl
  isotrivial} families of cubic surfaces to obtain quick relations on
the coefficients $a_{\bullet}$.  In order to implement these
isotrivial families, it is necessary to know whether a cubic surface
is represented by a point in the closure of $\Orb(F)$, $F$ general.
This is an instance of the much more general ``Orbit Closure Problem''
\cite[].
