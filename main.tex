...
\begin{figure}
    ...
    \caption{
      In the second test family, we blow up the plane at the points $\{s_{1}, \dots, s_{5}, t\}$ as the seven points $\{s_1, \dots, s_5, s_0, s_\infty\}$ vary freely on a fixed conic $C$.}
    \label{fig:FamilyB2}
  \end{figure}
...
By \autoref{prop:B1}, the $\mathcal Z \subset \mathcal P$ is a family of admissible subschemes.
We let
...
\begin{proof}[Proof of \autoref{lem:v2}]
  ...
  \end{equation}
  By the relative Euler sequence, we see that $\omega^{-1}_\rho$ is isomorphic to $\O_{\mathcal P}(3) \otimes \rho^* \mathcal L^3$, and hence the first term in \eqref{diff} is
