...
Let $R$ be a discrete valuation ring with residue field $\k$, fraction field $K$, and uniformizer $t$.
...
We are interested in the rational map $\phi \colon \P M \dashrightarrow \P N$.
...
Our goal is to explicitly describe a resolution of this rational map.
The analysis should hold for $\P^n$ for any $n$, but we stick to $n = 2$ to keep the exposition focused.

Fix a free $R$ module $M$ of rank 3.
Consider an isomorphism
\[ \phi \colon \P\left(M_K^\vee\right) \to \P^2.\]
Then $\phi$ is defined by a $K$-basis $(H_0, H_1, H_2)$ of $M_K$.
By multiplying by a suitable power of $t$
