...
Set 
\[Z' = Z \cup B_1 \times \{z_0\} \subset B_1 \times \P^2.\]
Our first family
\[\pi \colon X_1 \to B_1\]
is the family of cubic surfaces associated to $Z' \subset B_1 \times \P^2$ (see \autoref{def:cubicZ}).

We now compute the Chern classes of the push-forward of the anti-canonical bundle $\mathcal V$.
By construction, $\mathcal V$ is the rank 4 bundle
\[ \mathcal V = \pi_* (\mathcal I_{\mathcal Z} \otimes \omega_\pi^{-1}),\]
where $\pi \colon B_1 \times \P^2 \to B_1$ is the first projection.
In the Grothendieck group, we have
\begin{equation}\label{eqn:v11}
  \begin{split}
    [\mathcal V] &= \pi_*\left(\omega_\pi^{-1}\right) - \pi_*\left(\omega_\pi^{-1}|_{Z'}\right)\\
    &= \pi_*\left(\omega_\pi^{-1}\right) - \pi_*(\omega_\pi|_{B_1 \times \{z_0\}})- \pi_*\left(\omega_\pi^{-1}|_{Z}\right) \\
    &= [\O^{9}_{B_1}]- \pi_*\left(\omega_\pi^{-1}|_{Z}\right) .
  \end{split}
\end{equation}
Observe that $Z \subset B_1 \times C$ is a hypersurface of class $(1,5)$, and $\omega_\pi^{-1}$ on $B_1 \times C$ is isomorphic to $\O(0,6)$.
By applying $\pi_*$ to the exact sequence
\[ 0 \to \O(-1,1) \to \O(0,6) \to \O(0,6)|_Z \to 0,\]
we get
\begin{equation}\label{eqn:v12}
  \begin{split}
    \pi_*\left(\omega_\pi^{-1}|_{Z}\right) &= \pi_* \O(0,6) - \pi_* \O(-1,1)\\
    &= [\O_{B_1}^7] - [\O_{B_1}(-1)^2].
  \end{split}
\end{equation}
By combining \eqref{eqn:v11} and \eqref{eqn:v12}, we get
\begin{equation}\label{eqn:V1}
  [\mathcal V] = [\O_{B_1}^2] + [\O_{B_1}(-1)^2].
\end{equation}
\begin{lemma}
  For the bundle $\mathcal V$ on $B_1$, we have
  \[
    \int_{B_{1}}v_{1}^{4} = 16, \quad \int_{B_{1}}v_{1}^{2}v_{2} = 4,\quad \int_{B_{1}}v_{1}v_{3} =0,\quad \int_{B_{1}}v_2^{2} = 1,\quad \int_{B_{1}}v_{4} = 0.\]
\end{lemma}
\begin{proof}
  Follows from $[\mathcal V] = [\O^2] + [\O(-1)^2]$ after a straightforward computation.
\end{proof}
