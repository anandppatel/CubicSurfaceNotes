...
We now address the issue of goodness of the family $\pi \colon \mathcal X \to \widehat \M_{0,7}$.
Let $T \subset \widehat \M_{0,7}$ be the locus of curves $(C, s_0, s_1, \dots, s_5, s_\infty)$ where three among the $s_1,\dots, s_5$ coincide with $s_\infty$.
...
\begin{proof}
  ...
  differently, and appeals to the classification of semistable cubic
  surfaces.
  ...
  singularities.  Hence $X_{Z}$ is GIT semistable.  Semistable
...
The following is an instance of Brieskorn's ``simultaneous resolution of singularities'', see \cite{bri:70}:
...
\subsection{The anatomy of linear rational maps $\P_t^2 \dashrightarrow \P_t^2$}
...
We use the analysis above to understand limits of a general set of six points in $\P^2$ under a one-parameter family of automorphisms.
...
Recall that $R$ is a $\k$-algebra DVR with fraction field $K$.
We assume, as usual, that $R$ is a $\k$-algebra.
\begin{proposition}\label{prop:limit4}
  Let $Z \subset \P^2$ be a set of 6 points forming a star-free configuration.
  Given $\phi \in \Aut(\P^2_K)$, let $W \subset \P^2$ be the flat limit of $\phi(Z_K) \subset \P^2_K$.
  If $\phi$ has rank 2, then there exists a line in $\P^2$ containing at least 4 distinct points of $W$.
  If $\phi$ has rank 1, then there exists a point in $\P^2$ at which $W$ has multiplicity at least 4.
\end{proposition}
\begin{proof}
  Consider the rational map
  \[ \phi \colon \P^2_R \dashrightarrow \P^2_R.\]
  We analyze the limit of $\phi$ applied to the constant subscheme $Z_R \subset \P^2_R$.

  Note that we only need information about $W$ as a cycle.
