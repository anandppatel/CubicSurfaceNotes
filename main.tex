...
\begin{proposition}\label{prop:nolimit}
  ...
  Let $X \subset \P^3$ be a cubic surface whose stabilizer group in $\PGL(4)$ is reductive and such that for some $\mathbb G_m$-fixed hyperplane $H \subset \mathbb P^3$, the pair $(X, H)$ is $t$-semistable.
  Then $X$ does not lie in the closure of the $\SL_4$-orbit of $S$.
\end{proposition}

For the proof, we need a slightly technical lemma.
\begin{lemma}
  Suppose $X$ is a cubic surface whose stabilizer group $H \subset \PGL(4)$ is reductive.
  If $X$ lies in the closure of the $\PGL(4)$ orbit of a cubic surface $S$, then there exists a one-parameter subgroup $\lambda \colon \mathbb G_m \to H$ and a cubic surface $S'$ isomorphic to $S$ such that we have
...
\begin{proof}
  ...
  By \cite[Theorem~3]{alp:10}, there exists a $H$-invariant locally closed affine $W \subset \mathbb P \Sym^3 (4)$ containing the point $[X]$ such that the map
  \[ \pi \colon [W/H] \to [\mathbb P \Sym^3(4) / \PGL(4)]\]
  is affine and \'etale.
  (Note that we need only this result, which is substantially easier than the main theorem of \cite{alp.hal.ryd:20}.)
  Define $Z$ as the fiber product
  \[
    \begin{tikzcd}
      Z \ar{r}\ar{d}& {[W/H]}\ar{d}{\pi}\\
      \spec \k \ar{r}{[S]}& {[\mathbb P \Sym^3(4) / \PGL(4)]}.
    \end{tikzcd}
  \]
  Since $\pi$ is representable, \'etale, and of finite type, $Z$ is a finite disjoint of copies of $\spec \k$.
  Let $s_1, \dots, s_n \in [W/H](\k)$ be the points so that $Z = \sqcup s_i$.
  Since the point $[X] \in [\mathbb P \Sym^3 (4) / \PGL(4)](\k)$ lies in the closure of $[S] \in [\mathbb P \Sym^3(4)/\PGL(4)]$ and the map $\pi$ is \'etale (and hence open), the point $[X] \in [W/H](\k)$ lies in the closure of the set $\{s_1,\dots, s_n\}$.
  But then $[X]$ lies in the closure of $s_i$ for some $i$.
  In other words, the point $[X] \in W(\k)$ lies in the closure of the $H$-orbit of a lift $s_i \in W(\k)$ of $s_i \in [W/H](\k)$.
  By the Hilbert--Mumford criterion \cite[Theorem~1.4]{kem:78}, there exists a one-parameter subgroup $\lambda \colon \mathbb G_m \to H$ such that we have the equation
  \begin{equation}\label{eqn:specialization}
    [X] = \lim_{t \to 0} \lambda (t) \cdot s_i
  \end{equation}
  in $W(\k)$.
  Since $s_i \in [W/H](k)$ maps to $[S]$, the cubic surface $S'$ represented by $s_i \in W(\k)$ is isomorphic to $S$.
  Equation~\eqref{eqn:specialization} then yields the claim.
\end{proof}

\begin{proof}[Proof of \autoref{prop:nolimit}]
  Suppose $X$ lies in the orbit closure of $S$.
  Let $H \subset \PGL(4)$ be the stabilizer of $X$.
  Then there exists a one-parameter subgroup $\alpha \colon \mathbb G_m \to H$ and a cubic surface $Y \subset \P^3$ such that $Y$ is in the $\PGL(4)$-orbit of $S$ and
...
\begin{proof}
  ...
  This proof needs to be fixed. For that we need information about which hyperplane sections are semi-stable.
  \begin{enumerate}
  \item $x_0x_1x_3 = x_2^3$. \quad All $H$.
  \item $x_3(x_0x_2-x_1^2) = x_0x_1^2$. \quad 
  \item $x_3(x_0x_2-x_1^2) = x_0^2x_1$ \quad $(A_4 + A_1)$
  \item $x_3x_0^2 = x_1^3 + x_2^3$ \quad $(D_4)$
  \end{enumerate}
\end{proof}
