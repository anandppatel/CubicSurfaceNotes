...
Cubic surfaces have captured our imagination for centuries. Beginning
with Cayley and Salmon's discovery of the the 27 lines, a long list of
algebraic geometers have revealed intricate facets of their geometry,
using new and increasingly sophisticated tools. We add to this
development by answering a family of natural enumerative questions
about families of cubic surfaces.

It is well-known that the isomorphism classes of cubic surfaces form a
4-dimensional moduli space. So, we expect that in a 4-dimensional
family of cubic surfaces, a general isomorphism class would appear
finitely many times. How do we determine this number? For example, how
many times does a general cubic surface appear, up to isomorphism, as
\begin{enumerate}
\item \label{orbit} a member of a 4-dimensional general linear system of cubic surfaces in $\mathbb
  P^3$?
\item a hyperplane section of a general cubic threefold in $\mathbb
  P^4$?
\item a blow up of 6 points in $\mathbb P^2$ varying in a general
...
semi-stable cubic surfaces in $\mathbb P^3$ by the action of
...
notion includes all families whose fibers are GIT-semistable or have a finite automorphism group.

\subsection{Degree of the orbit closure}
Question~\ref{orbit} in \autoref{sec:counting} is equivalent to a question
that has been an ongoing topic of investigation.
Consider a hypersurface \(X\) in a projective space $\mathbb P V$ cut out by
...
that is, when \(X\) is the set of points in $\mathbb P^1$ or a curve
in $\mathbb P^2$ \cite{alu.fab:00,alu.fab:93,alu.fab:93*1}.
The next unknown case is that of cubic surfaces in $\mathbb P^3$.
The answer to Problem~\ref{orbit} says that the degree of the
...
``numerical proof'' for the number 96120 and in 2020, Cazzador and
Skauli outlined an approach towards a complete proof following the
...
The key idea is to consider not just the degree of the orbit closure,
but the class of the orbit closure in the equivariant Chow ring
$A^\bullet_{\GL V}(\Sym^dV)$, or equivalently, in the Chow ring of the
stack $[\Sym^dV/\GL V]$. The problem of finding the equivariant class
of the orbit closure subsumes the original problem of finding its
degree.  It turns out that, counter-intuitively, the more
general problem is easier to solve---it is amenable to techniques that
were not originally available.

How does the equivariant class yields the degree of the
orbit?
...
\begin{tikzcd}
  ...
  \{0\}) \ar[equal]{d}\\
  &A^\bullet_{\PGL V}(\mathbb P \Sym^dV) \ar{r}&
  A^\bullet(\mathbb P \Sym^dV).
\end{tikzcd}
\]
Under the composite map, the class of $\Orb(F) \in A^\bullet_{\GL
  V}(\Sym^dV)$ is mapped to the class of $\Orb([F]) \in
A^\bullet(\mathbb P \Sym^dV)$, which encodes simply the degree of $\Orb([F])$.

Why makes it possible to find the equivariant class?  The ring
$A^\bullet_{\GL V}(\Sym^dV)$ is the polynomial ring in variables
$c_1, \dots, c_r$, where $r = \dim V$; the variable $c_i$ has degree
$i$.  The equivariant class of $\Orb(F)$ lies in the graded component
$A^m(\Sym^dV)$, where $m = \dim \Sym^d V - \dim \GL V$.  As a result,
it can be expressed as a $\mathbb Z$-linear combination of monomials
in $c_i$ of total degree $m$. We can then use the method of
undetermined coefficients.
We construct maps $B \to [\Sym^dV/\GL V]$ where the pull-back
of $\Orb(F)$ as well as the classes $c_i$ can be explicitly computed.
Enough families give enough information to find all the coefficients.
For cubic surfaces, we write more than enough families---there are 5
coefficients to be determined; we write 7 families.
Being able to write enough families where both sides can be
computed is the main obstacle in solving the question for
...
\begin{lemma}
  ...
  \begin{align}
    \label{eq:rnk10}
    \rho_{*}(\omega_{\rho}^{-1}) \simeq \Sym^{3}(\Sym^{2} \mathcal{A}^{\smvee})\otimes \mathcal{L}^{3}.
  \end{align}
\end{lemma}
...
\begin{proof}
  ...
  \begin{align}
    \label{eq:eulerexact}
    0 \to \Omega_{\mathcal{P}/\o{\M}_{0,7}} \to \rho^{*}(\Sym^{2}\mathcal{A})(-1) \to \O_{\cP} \to 0.
  \end{align}
\end{proof}

\begin{proposition}
  \label{prop:omegarestrictZ} 
\end{proposition}
