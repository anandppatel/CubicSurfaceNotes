...
\begin{theorem}\label{theorem:main}
  ...
  $\mu \colon B \dashrightarrow \mathcal M$ be the induced moduli map.
  Then,
  \begin{align}
    \deg \mu =
    \label{eq:MAIN}
...
\begin{example}[Fibers with finite automorphism groups]
  ...
  contain $\cX_b$.
\end{example}
\begin{example}[GIT semi-stable fibers]
  \label{ex:semistable}
  Suppose every fiber $\cX_b$ is GIT-semi-stable.
...
relation on the indetermined coefficients in
...
Finally, {\sl we assume the reader is familiar with the intersection
  theory of $\o{\M}_{0,n}$.}  Therefore, we will at times translate
intersection products on $\hM_{0,7}$ into calculations on
...
\begin{proof}
  Each surface $X$ in this list has a reductive stabilizer group $T$, and there exists a $T_0$-fixed hyperplane section $H$ such that the pair $(X, H)$ is $t$-semi-stable for some $0 < t \leq 3/7$.

  In the following table, we list the automorphism groups, the hyperplane $H$, and the relevant value of $t$.
  The automorphism groups are from \cite[Theorem~3]{sak:10} and the data of the $t$-semi-stable hyperplane is from \cite[Table~2]{gal.mar:19}.
...
>>>>>>> 7364e5f1c63c53ca7501ae95b1608ddebfd41b2c
...
The following is an instance of Brieskorn's ``simultaneous resolution of singularities'':
...
\begin{proof}
  See \cite{brieskorn1970singular}.
\end{proof}
